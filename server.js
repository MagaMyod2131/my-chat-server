...
server.on('connection', function(ws) {
  ...
  ws.on('message', function(raw) {
    ...
    switch (msg.type) {
      ...
      case 'find_user':
        var found = null;
        if (msg.query && users[msg.query] && msg.query !== ws.userId) {
          found = { userId: msg.query, name: userNames[msg.query], online: true };
        }
        if (!found) {
          for (var uid in userNames) {
            if (uid !== ws.userId && userNames[uid] && userNames[uid].toLowerCase() === (msg.query||'').toLowerCase()) {
      ...
      case 'dm':
        var k1 = dmKey(ws.userId, msg.toId);
        var dm = { type: 'dm', fromId: ws.userId, toId: msg.toId, name: ws.name, text: msg.text, time: msg.time, id: msg.id, replyTo: msg.replyTo || null };
        saveDM(k1, dm);
      ...
      case 'dm_file':
        var k2 = dmKey(ws.userId, msg.toId);
        var dmf = { type: 'dm_file', fromId: ws.userId, toId: msg.toId, name: ws.name, fileType: msg.fileType, fileName: msg.fileName, fileSize: msg.fileSize, data: msg.data, time: msg.time, id: msg.id };
        saveDM(k2, dmf);
      ...
      case 'dm_edit':
        var k3 = dmKey(ws.userId, msg.toId);
        if (dmHistory[k3]) {
          for (var i = 0; i < dmHistory[k3].length; i++) {
            if (dmHistory[k3][i].id === msg.id && dmHistory[k3][i].fromId === ws.userId) {
              dmHistory[k3][i].text = msg.text; dmHistory[k3][i].edited = true; break;
            }
          }
        }
        var ep = { type: 'dm_edit', fromId: ws.userId, toId: msg.toId, id: msg.id, text: msg.text };
        ws.send(JSON.stringify(ep)); sendTo(msg.toId, ep);
        break;

      case 'dm_delete':
        var k4 = dmKey(ws.userId, msg.toId);
        if (dmHistory[k4]) {
          for (var j = 0; j < dmHistory[k4].length; j++) {
            if (dmHistory[k4][j].id === msg.id && dmHistory[k4][j].fromId === ws.userId) { dmHistory[k4].splice(j,1); break; }
          }
        }
        var dp = { type: 'dm_delete', fromId: ws.userId, toId: msg.toId, id: msg.id };
        ws.send(JSON.stringify(dp)); sendTo(msg.toId, dp);
        break;
      ...
      case 'dm_clear':
        ...
        ws.send(JSON.stringify(cp)); sendTo(msg.toId, cp);
        break;

      case 'dm_typing':
        sendTo(msg.toId, { type: 'dm_typing', fromId: ws.userId, name: ws.name, isTyping: msg.isTyping, isSending: msg.isSending || false });
        break;

      case 'call_offer':
        sendTo(msg.toId, { type: 'call_offer', fromId: ws.userId, fromName: ws.name, sdp: msg.sdp, callType: msg.callType || 'audio' });
        break;
      case 'call_answer':
      ...
      case 'group_call_join':
        broadcast(msg.room, { type: 'group_call_join', fromId: ws.userId, fromName: ws.name }, ws);
        break;
      case 'group_call_offer':
        sendTo(msg.toId, { type: 'group_call_offer', fromId: ws.userId, fromName: ws.name, sdp: msg.sdp, callType: msg.callType || 'audio' });
        break;
      ...
      case 'group_call_leave':
        broadcast(msg.room, { type: 'group_call_leave', fromId: ws.userId, fromName: ws.name }, ws);
        break;
      ...
      case 'message':
        ...
        saveMsg(ws.room, gm); broadcastAll(ws.room, gm);
        break;
      ...
      case 'file':
        ...
        saveMsg(ws.room, gf); broadcastAll(ws.room, gf);
        break;
        ...
        if (history[ws.room]) {
          for (var ei = 0; ei < history[ws.room].length; ei++) {
            if (history[ws.room][ei].id === msg.id && history[ws.room][ei].name === ws.name) {
              history[ws.room][ei].text = msg.text; history[ws.room][ei].edited = true; break;
            }
          }
        }
      ...
      case 'delete':
        if (history[ws.room]) {
          for (var di = 0; di < history[ws.room].length; di++) {
            if (history[ws.room][di].id === msg.id && history[ws.room][di].name === ws.name) { history[ws.room].splice(di,1); break; }
          }
        }
      ...
      case 'typing':
        broadcast(ws.room, { type: 'typing', room: ws.room, name: ws.name, isTyping: msg.isTyping, isSending: msg.isSending || false }, ws);
        break;
    }
  });
